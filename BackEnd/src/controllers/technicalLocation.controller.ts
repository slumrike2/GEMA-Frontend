...
export const technicalLocationController = {
	...
	async create(req: Request, res: Response) {
		...
		try {
			...
			const result = await db
				.insert(technicalLocation)
				.values({
					technicalCode: tc,
					name: name,
					type: type,
					abbreviatedTechnicalCode: code,
					parentTechnicalCode: ptc
				})
				.returning();
	...
	async getRoots(req: Request, res: Response) {
		try {
			// Busca las ubicaciones que no tienen padre (es decir, son raíces)
			const results = await db
				.select()
				.from(technicalLocation)
				.where(
					or(
						isNull(technicalLocation.parentTechnicalCode),
						eq(technicalLocation.technicalCode, '') // Excluye la ubicación Sede
					)
				);

			console.log(results);
			if (results.length === 0) {
				res.status(404).json({ error: 'Technical location not found' });
			}
